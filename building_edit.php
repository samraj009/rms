...
$page_title = "Edit Building Particulars";
$msg = "";
$error_msg = "";
...
if( ISSET($_POST['save_button']) ) {
	...
	if( !empty($_POST['building_owner']) && !empty($_POST['building_complex']) ) {
		$result = building_row_update();
		$msg = "Building particulars edited";
	} else {
		$error_msg = "Please enter input fields";
	}	
} 

$params['building_id'] = $_GET['id'];
$data = building_row_select_single($params);
$result = isset($data[0]) ? $data[0] : array();
//echo "<pre>";print_r($result);echo "</pre>";

$building_owner = ISSET($result['building_owner']) ? $result['building_owner']: '';
...
if( count($owner_lists) > 0 ){
	foreach($owner_lists as $row){
		if($building_owner == $row['owner_id']){ 
		$selected = "selected='selected'";
		} else {
			$selected = "";
		}
	$onwer_list .= '<option value="'.$row['owner_id'].'" '.$selected.'>'.$row['owner_name'].'</option>';
} // END: FOR EACH
} // END : IF
...
<body id="page-top">
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
			...
			<div class="form-group col-md-4">
			  ...
			  <input type="text" class="form-control" id="building_complex" name="building_complex" placeholder="Enter Complex Name" value="<?php echo ISSET($result['building_complex']) ? $result['building_complex']: '';?>" />
			</div>
			
			<div class="form-group col-md-4">
			  <label for="building_placeno">No.of Rental place</label>
			  <input type="text" class="form-control" id="building_placeno" name="building_placeno" placeholder="Enter No.of Rental Place" value="<?php echo ISSET($result['building_placeno']) ? $result['building_placeno']: '';?>" />
			</div>
  ...
  <div class="form-row">
    <div class="form-group col-md-4">
      ...
      <input type="text" class="form-control" id="building_watertax" name="building_watertax" placeholder="Enter water Tax" value="<?php echo ISSET($result['building_watertax']) ? $result['building_watertax']: '';?>" />
    </div>
	<div class="form-group col-md-4">
      <label for="building_corporationtax">Corporation Tax</label>
      <input type="text" class="form-control" id="building_corporationtax" name="building_corporationtax" placeholder="Enter Corporation Tax" value="<?php echo ISSET($result['building_corporationtax']) ? $result['building_corporationtax']: '';?>" />
    </div>
    <div class="form-group col-md-4">
      <label for="building_ebdeposit">EB Deposit Amount</label>
      <input type="text" class="form-control" id="building_ebdeposit" name="building_ebdeposit" placeholder="Enter EB Deposit Amount" value="<?php echo ISSET($result['building_ebdeposit']) ? $result['building_ebdeposit']: '';?>" />
    </div>
  </div>
  ...
  <div class="form-group col-md-4">
	...
	<textarea class="form-control" id="building_address" name="building_address" placeholder="Enter Building Address" rows="3"><?php echo ISSET($result['building_address']) ? $result['building_address']: '';?></textarea>
  </div>
  
  <div class="form-row">
    <div class="form-group col-md-3">
      <label for="building_city">City</label>
      <input type="text" class="form-control" id="building_city" name="building_city" value="<?php echo ISSET($result['building_city']) ? $result['building_city']: '';?>" />
    </div>
	<div class="form-group col-md-3">
      <label for="building_district">District</label>
      <input type="text" class="form-control" id="building_district" name="building_district" value="<?php echo ISSET($result['building_district']) ? $result['building_district']: '';?>" />
    </div>
    <div class="form-group col-md-3">
    ...
    <div class="form-group col-md-3">
      ...
      <input type="text" class="form-control" id="building_zipcode" name="building_zipcode" value="<?php echo ISSET($result['building_zipcode']) ? $result['building_zipcode']: '';?>" />
    </div>
  </div>
